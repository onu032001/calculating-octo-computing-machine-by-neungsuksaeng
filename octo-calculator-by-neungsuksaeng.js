...
class NeungsuksaengCalculateParser {
  ...
  parseExponent() {
    ...
    while (this.currentToken?.token === '^') {
      ...
      result = { type: 'operation', operator: operator.token, left: result, right: nextFactor };
    }
    return result;
  }
  parseTerm() {
    ...
    while (this.currentToken?.token === '*' || this.currentToken?.token === '/') {
      ...
      result = { type: 'operation', operator: operator.token, left: result, right: nextExponent };
    }
    return result;
  }
  parseExpression() {
    ...
    while (this.currentToken?.token === '+' || this.currentToken?.token === '-') {
      ...
      result = { type: 'operation', operator: operator.token, left: result, right: nextTerm };
    }
    return result;
  }
}
